...
if(isset($_POST['save']))
{
	...
	include "../_/components/administration/php-version/actions/editProduct.action.php";
}else{
	if(isset($_POST['cancel'])){
		include "../_/components/administration/php-version/Product_grid.php";
	}else{
		getItem($_REQUEST['id']);
		getCategory($_REQUEST['id']);
...
function getCategory($productId){
	$object = new ProdXCategory();
	$DAO = DAOFactory::getProdXCategoryDAO();
	$object = $DAO->getActiveProdXCategoryByProductOrder($productId);
	if(isset($object) && isset($object[0])){
		$_SESSION['pc_category'] = $object[0]->category;
	}
}
...
function getItem($id){
	...
	if(isset($object)){
		...
		if($object->image != ''){
			...
			$_SESSION['pc_image_location']	= '../images/product/'.$object->id.'/'.$object->image;
		}
