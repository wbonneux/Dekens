		...
		<div class="box-content">
			...
			<table
				...
				for($i = 0; $i < count ( $daysClosedArray ); $i ++) {
					...
					?>
					<td class='center'>van: <input type='time' name='hrs_am_begin_<?php echo $row->id; ?>' id='hrs_am_begin_<?php echo $row->id; ?>' value="<?php if(isset($_SESSION['hrs_am_begin_'.$row->id])){echo $_SESSION['hrs_am_begin_'.$row->id];}else{echo $row->hoursAmBegin;} ?>">tot: <input type='time' name='hrs_am_end_<?php echo $row->id; ?>' id='hrs_am_end_<?php echo $row->id; ?>' value="<?php if(isset($_SESSION['hrs_am_end_'.$row->id])){echo $_SESSION['hrs_am_end_'.$row->id];}else{echo $row->hoursAmEnd;} ?>"></td>
					<td class='center'>van: <input type='time' name='hrs_pm_begin_<?php echo $row->id; ?>' id='hrs_pm_begin_<?php echo $row->id; ?>' value="<?php if(isset($_SESSION['hrs_pm_begin_'.$row->id])){echo $_SESSION['hrs_pm_begin_'.$row->id];}else{echo $row->hoursPmBegin;} ?>">tot: <input type='time' name='hrs_pm_end_<?php echo $row->id; ?>' id='hrs_pm_end_<?php echo $row->id; ?>' value="<?php if(isset($_SESSION['hrs_pm_end_'.$row->id])){echo $_SESSION['hrs_pm_end_'.$row->id];}else{echo $row->hoursPmEnd;} ?>"></td>
					<td class='center'>van: <input type='time' name='hrs_dy_begin_<?php echo $row->id; ?>' id='hrs_dy_begin_<?php echo $row->id; ?>' value="<?php if(isset($_SESSION['hrs_dy_begin_'.$row->id])){echo $_SESSION['hrs_dy_begin_'.$row->id];}else{echo $row->hoursDayBegin;} ?>">tot: <input type='time' name='hrs_dy_end_<?php echo $row->id; ?>' id='hrs_dy_end_<?php echo $row->id; ?>' value="<?php if(isset($_SESSION['hrs_dy_end_'.$row->id])){echo $_SESSION['hrs_dy_end_'.$row->id];}else{echo $row->hoursDayEnd;} ?>"></td>
 					<?php 
 					echo "</td>";
